...
const MAX_HISTORY_POINTS_DESKTOP = 96;
const MAX_HISTORY_POINTS_MOBILE = 48;
...
function getMaxHistoryPoints() {
    return window.innerWidth < 768 ? MAX_HISTORY_POINTS_MOBILE : MAX_HISTORY_POINTS_DESKTOP;
}

function downsampleHistory(history, targetPoints) {
    if (history.length <= targetPoints) return history.map(h => ({...h, isGroup: false}));
    ...
    for (let i = 0; i < targetPoints; i++) {
        ...
        const hasOffline = chunk.some(h => h.status !== 'online');
...
async function renderAvailability() {
    ...
    try {
        ...
        for (const nodeName in allHistory) {
            const rawHistory = allHistory[nodeName];
            if (!rawHistory || rawHistory.length === 0) continue;

            const onlineCount = rawHistory.filter(h => h.status === 'online').length;
            const uptimePct = ((onlineCount / rawHistory.length) * 100).toFixed(1);

            // Display logic with downsampling
            // We want to fill maxPoints slots. 
            // If rawHistory has fewer points than maxPoints, we show them as is, and pad with empty.
            // If rawHistory has more points, we might need to downsample if we want to represent a longer period, 
            // BUT the requirement is usually to show the *same* time window but with less granularity on mobile,
            // OR show a shorter time window? 
            // Assuming the API returns a fixed number of points (e.g. 96 points for 24h).
            // If API returns 96 points, and we want 48 points on mobile, we merge every 2 points.
            
            let displayHistory = rawHistory;
            if (rawHistory.length > maxPoints) {
                 displayHistory = downsampleHistory(rawHistory, maxPoints);
            }

            let segmentsHtml = '';
            // Pad with empty segments if we don't have enough data to fill the view
            // Assuming maxPoints represents the full "window" (e.g. 24h)
            const emptyPoints = Math.max(0, maxPoints - displayHistory.length);
            for (let i = 0; i < emptyPoints; i++) {
                segmentsHtml += '<div class="uptime-segment none"><span class="tooltip-text">无数据</span></div>';
            }

            displayHistory.forEach(point => {
                let timeStr;
                if (point.isGroup && point.startTime) {
                     timeStr = formatTimeRange(point.startTime, point.time);
                } else {
                     timeStr = formatTimestamp(point.time);
                }
            ...
            html += `
                <div class="uptime-card">
                    ...
                    <div class="uptime-footer">
                        <span>${formatDuration(MAX_HISTORY_POINTS_DESKTOP * 15)}前</span>
                        <span>最近</span>
                        <span>现在</span>
                    </div>
                </div>
